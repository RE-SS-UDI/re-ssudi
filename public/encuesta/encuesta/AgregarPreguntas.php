<?php $titulo="Agregar Preguntas a "; ?>
...
<?php
	...
	if( !empty($_GET["pre"]) ){		
		...
		if ( $resultadoExiste->num_rows == 0){
			$sql = $conn->prepare("INSERT INTO pregunta_encuesta (Pregunta_idPregunta, Encuesta_idEncuesta) VALUES (?,?)");
		...
		<script> alert('Pregunta existente en la encuesta')</script>
		<?php
		}
	}
			...
			<?php
				...
				$sql = "SELECT 	DISTINCT p.idPregunta, p.descripcionPregunta, t.idTipo, t.descripcionTipo
						...
						WHERE 	p.elimina_pregunta =0
								AND p.Categoria_idCategoria ='".$categoriaSeleccionada."'";
				...
				?>
						<tr>
							...
							<td><a href="<?php echo htmlspecialchars($_SERVER["PHP_SELF"])."?id=".$idModificar."&ini=".$inicio."&cat=".$categoriaSeleccionada."&pre=".$row["idPregunta"];?>">
