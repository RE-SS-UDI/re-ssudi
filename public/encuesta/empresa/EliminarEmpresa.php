...
<?php
    ...
    if(empty($_GET["id"])){
?>
        <form method="get" action="EliminarEmpresa.php">
...
?>
        ...
        <form action="EliminarEmpresa.php">
...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$sqlActualizar = "UPDATE empresa SET elimina=1 where idEmpresa='".$idModificar."'";
...
?>
     <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"])."?id=".$idModificar;?>">
        ...
        <label>Usuario (id usuario)</label><br>
        <label><?php echo $obj->idUsuario->valor;?></label><br><br>
...
<?php }
		...
		$sql = "SELECT nombreUsuario FROM usuario WHERE and elimina=0 idUsuario = '".$obj->idUsuario->valor."'";
		echo $sql;
        $resultadoUsuario = $conn->query($sql);
...
Se eliminó la empresa " <?php echo $obj->razonSocial->valor;?> " del usuario " <?php echo $usuario["nombreUsuario"] ?> " con éxito 
